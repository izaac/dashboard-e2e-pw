...
- **Import paths tell the story**, so scan imports before reading the whole file.
- **`tsconfig.json` paths** define the `@/` aliases, so don't resolve manually.
- **Playwright types**: `Page`, `Locator`, and `expect` are the only 3 you need. Don't load Playwright internals.
...
- **One spec at a time**, so run `-g "test name"` to isolate failures, not the whole suite.
- **Page Objects are the docs**, so read the PO, not the Playwright API docs, for selector patterns.
- **Check existing POs first** by reading `e2e/po/INDEX.md` before creating. It lists every PO with class, selector, and
...
- **Skip upstream comments**, so don't copy eslint-disable, TODO, or commented-out Cypress code.
- **Upstream spec → Playwright spec is mechanical**, so see the Conversion Patterns table below.
...
- **Use background agents** for independent work, and don't block main context waiting.
- **Don't re-read files agents already wrote.** Trust agent output summaries.
- **Kill stale context** by running `/compact` after agent reports land, before starting new work.
...
- **Pattern:** Page Object Model (PO), which mirrors upstream Cypress structure
...
`playwright.config.jenkins.ts`, and specs don't need any Qase-specific code until QA maps them.
...
- **Cloud provider creds**: AWS (`awsAccessKey`/`awsSecretKey`), Azure
  ...
  these.
- **Custom nodes**: `customNodeIp` / `customNodeKey`, since RKE1/custom cluster tests need real nodes.
...
- If a resource might already exist, check first, and don't fail on "already exists"
...
- **No manual waits around assertions**, so don't wrap `expect()` in try/catch. Playwright assertions auto-retry. Only
...
- Factory functions return minimal fields the UI needs to render (name, namespace, state) with no hardcoded URLs, no
  Cypress revision constants
- Use `page.route(url, route => route.fulfill({ json }))`, where the `json` param handles serialization + content-type
  automatically
- `page.route()` MUST be set up BEFORE `await login()`, since the route needs to be active before the page loads
...
- **Known:** `RulePath.vue` (ingress rules) has a 500ms debounce on `update()` and `updatePathTypeAndPath()`
...
- **Known affected:** Rancher setup page (`shell/pages/auth/setup.vue`),
  where `password` data is initialized to a random string, and Vue's
...
- **Check existing POs via `e2e/po/INDEX.md`** before creating new ones, and do NOT `find` or `grep` for POs.
- **Check upstream parity via `yarn po-diff`** before manually comparing POs, and do NOT read upstream files one by one.
...
- Use subagents for codebase exploration, which keeps main context clean.
- Never load full docs when a link or `@` import suffices.
- Point to specific files and line numbers in prompts, because vague = expensive.
