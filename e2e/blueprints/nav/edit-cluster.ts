...
export function generateFakeNodeDriversReply(): any {
  return {
    ...
    metadata: {
      annotations: {
        'io.cattle.nodedriver/ui-field-hints':
          '{"sshKeyContents":{"type":"multiline"},"userdata":{"type":"multiline"}}',
    ...
    spec: {
      ...
      uiUrl: '',
      url: 'local://',
      whitelistDomains: ['api.digitalocean.com'],
    },
    status: {
      appliedChecksum: '',
...
export function generateFakeMachineConfigReply(provClusterId: string, machinePoolId: string): any {
  return {
    ...
    metadata: {
      ...
      fields: [`nc-${provClusterId}-pool1-${machinePoolId}`, '27h'],
