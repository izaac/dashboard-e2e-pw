...
test.describe('Feature Flags', () => {
  ...
  test('can toggle harvester feature flag', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
  ...
  test('can toggle harvester-baremetal-container-workload feature flag', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
    const featureFlagsPage = new FeatureFlagsPagePo(page);
  ...
  test('can toggle istio-virtual-service-ui feature flag', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
    const featureFlagsPage = new FeatureFlagsPagePo(page);
  ...
  test('can toggle rke1-custom-node-cleanup feature flag', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
    const featureFlagsPage = new FeatureFlagsPagePo(page);
  ...
  test('can toggle token-hashing feature flag', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
    const featureFlagsPage = new FeatureFlagsPagePo(page);
  ...
  test('can toggle unsupported-storage-drivers feature flag', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
    const featureFlagsPage = new FeatureFlagsPagePo(page);
    ...
    // Activate
    await featureFlagsPage.list().resourceTable().sortableTable().rowWithName(flagName).self().scrollIntoViewIfNeeded();
    await featureFlagsPage.list().clickRowActionMenuItem(flagName, 'Activate');
    await featureFlagsPage.clickCardActionButtonAndWait('Activate', flagName, true);
  ...
  test('error when toggling a feature flag is handled correctly', { tag: ['@globalSettings', '@adminUser'] }, async ({ page, rancherApi }) => {
    const featureFlagsPage = new FeatureFlagsPagePo(page);
    const flagName = 'unsupported-storage-drivers';
  ...
  test('standard user has only read access to Feature Flag page', { tag: ['@globalSettings', '@standardUser'] }, async ({ page, login }) => {
  ...
  test.describe('List', { tag: ['@noVai', '@globalSettings', '@adminUser', '@standardUser'] }, () => {
    test('validate feature flags table header content', async ({ page, rancherApi }) => {
      ...
      const headers = featureFlagsPage.list().resourceTable().sortableTable().self()
