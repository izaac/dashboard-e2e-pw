...
test.describe('Create EKS cluster', { tag: ['@manager', '@adminUser', '@provisioning'] }, () => {
  ...
  test('can re-name node pools without changing the order in which they are displayed in the UI', async ({
    ...
    const credName = rancherApi.createE2EResourceName('ekscloudcredential3');
    const clusterList = new ClusterManagerListPagePo(page);
    const createEKSClusterPage = new ClusterManagerCreateEKSPagePo(page);
    let cloudCredId = '';
    ...
    try {
      ...
      const normalizedInitialTabNames = initialTabNames.map((n) => n.trim());

      // Add a new tab
      await tabbedPo.self().locator('[data-testid="tab-list-add"]').click();

      const tabNamesAfterAdd = await tabbedPo.tabNames();
      const normalizedTabNamesAfterAdd = tabNamesAfterAdd.map((n) => n.trim());
      const lastTabAfterAdd = normalizedTabNamesAfterAdd[normalizedTabNamesAfterAdd.length - 1];

      expect(normalizedTabNamesAfterAdd.length).toBe(normalizedInitialTabNames.length + 1);
      expect(lastTabAfterAdd).not.toBe('');

      // Click 2nd tab (index 1) and rename
      await tabbedPo.clickNthTab(2);
      await createEKSClusterPage.getNodeGroup().set('aaa');

      const tabNamesAfterRename = await tabbedPo.tabNames();
      const normalizedTabNamesAfterRename = tabNamesAfterRename.map((n) => n.trim());
      const lastTabAfterRename = normalizedTabNamesAfterRename[normalizedTabNamesAfterRename.length - 1];

      expect(normalizedTabNamesAfterRename.length).toBe(normalizedTabNamesAfterAdd.length);
